$(document).ready(function(){
	...
	*/
	$("body").delegate(".qty","keyup",function(event){
		...
		var total = price * qty*total_duration;
	...
	*/
	$("body").delegate(".update","click",function(event){
		...
		var end_date=update.find(".end_date").val();
		
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{updateCartItem:1,update_id:update_id,qty:qty,start_date:start_date,end_date:end_date},
			success	:	function(data){
				$("#cart_msg").html(data);
				checkOutDetails();
			}
	...
	*/
	function net_total(){
		...
		$('.qty').each(function(){
			...
			var price  = row.find('.price').val();
			var total_duration = row.find('.total_duration').val();
			var total = price * $(this).val()*total_duration;
			row.find('.total').val(total);
		})
		$('.total').each(function(){
			net_total += ($(this).val()-0);
